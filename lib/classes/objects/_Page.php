...
class _Page {
    ...
    function _on_load() {
        ...
        if ($this -> system -> response_type == 'html')
            $this -> assign('p', $this -> system -> get_page());
    }

    function _has_action($name) {
        return method_exists($this, 'action_' . $name) || method_exists($this, $this -> system -> request_method . '_action_' . $name);
    }
    ...
    function _init() {
        ...
        if ($this -> subquery && isset($this -> subquery[0]) && $this -> subquery[0]) {
            ...
            } elseif ($this -> _has_action($this -> subquery[0])) {
    ...
    function _render_template($render_type = 'all') {
        ...
        if ($render_type == 'all' || $render_type == $this -> template_var) {
            ...
            elseif (is_dir($this -> folder . 'views/' . $this -> system -> default_skin . '/'))
            ...
            if ($this -> subpage && TemplatesManager::get_template_var($this -> subpage -> template_var))
                $this -> assign('subpage', TemplatesManager::get_template_var($this -> subpage -> template_var));
            $template_dir = $path;
        ...
        if ($this -> template_var == 'page_component_2')
            $this -> assign ('page_subcomponent', TemplatesManager::get_template_var('page_component_2'));
        if ($this -> template_var == 'page_component_1')
            $this -> assign('page_component', TemplatesManager::get_template_var('page_component_1'));
        if ($this -> template_var == 'page_component_0')
            $this -> assign('page_content', TemplatesManager::get_template_var('page_component_0'));
