...
class SimpleExcel
{
	public static function Export($file_name='export.xls',$data=array())
	{
		// Send Header
        ...
        foreach($data as $k=>$row)
        {
        	...
        	foreach($row as $v)
        	{
        		SimpleExcel::xlsWriteLabel($k,$col,$v);
        		$col++;
        	}
        }
	...
	public static function Import($file='',$has_header=1)
	{
		if ($file)
		{
			$data = new Spreadsheet_Excel_Reader(); // create  class object

			$data->setOutputEncoding('CPa25a');

			$what_to_read = $file;

			$data->read($what_to_read);

			$max_rows = $data->sheets[0]['numRows'];

			$start=$has_header?2:1;

			$arr=array();
			for ($row=$start;$row<=$max_rows;$row++)
			{
				$arr[]=$data->sheets[0]['cells'][$row];
			}
			return $arr;
		}
		return false;
	...
	public static function Save($file_name='export.xls',$data=array())
	{
		ob_start();
