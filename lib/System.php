...
class System
{
    ...
    function init() {
        ...
        if ($this->db_conn_enabled) {
            ...
            // execute console
            if ($this->console) ConsoleManager::execute();
    ...
    function __autoload($name) {
        ...
        if (isset($results[0])) if (count($results[0])) $file = __DIR__ . '/classes/' . strtolower(array_pop($results[0])) . 's/' . $name . '.php';
        if (file_exists($file)) require_once $file;
        elseif (file_exists(__DIR__ . '/classes/objects/' . $name . '.php')) require_once __DIR__ . '/classes/objects/' . $name . '.php';
    }
    ...
    private function _init_system_error($code = '500', $message = '') {
        if ($this->system_error_enabled && ($this->system_error || $message)) {
            ...
            if (is_file($err_file) && $this->template) $this->template->display($err_file);
            else echo '<h1>' . $status . '</h1>' . $this->system_error;
            exit();
        }
    }
    ...
    private function _init_config() {
        global $page;
        if (defined('SYSTEM_CONFIG_FILE')) $this->config_file = SYSTEM_CONFIG_FILE;
        ...
        // configuration file
        if (isset($_SERVER["SCRIPT_FILENAME"]) && is_file(dirname($_SERVER["SCRIPT_FILENAME"]) . DS . $this->config_file)) include_once dirname($_SERVER["SCRIPT_FILENAME"]) . DS . $this->config_file;
        elseif (defined('SYSTEM_CONFIG_PATH') && is_file(SYSTEM_CONFIG_PATH . $this->config_file)) include_once SYSTEM_CONFIG_PATH . $this->config_file;
        else trigger_error('Configuration file "' . dirname($_SERVER["SCRIPT_FILENAME"]) . DS . $this->config_file . '" missing!');
        
        // get environment
        $configs = $this->get_configs();
        foreach ($configs as $env) if (isset($_SERVER["SCRIPT_FILENAME"]) && is_file(dirname($_SERVER["SCRIPT_FILENAME"]) . DS . 'config.' . $env . '.php')) include_once dirname($_SERVER["SCRIPT_FILENAME"]) . DS . 'config.' . $env . '.php';
        else trigger_error('Configuration file "' . dirname($_SERVER["SCRIPT_FILENAME"]) . DS . 'config.' . $env . '.php" missing!');
        
        if (substr($this->default_module, -1) !== '/') $this->default_module.= '/';
        $this->_init_debug();
    }
    ...
    private function _init_template() {
        ...
        if ($this->template_engine == 'smarty') $this->import('library', 'wbl_smarty');
    ...
    private function _init_dal() {
        ...
        if ($this->admin) $this->import('class', 'objects.BaseAdmin');
        else $this->import('class', 'objects.Base');
    ...
    private function _init_module_config() {
        ...
        if (defined('MODULE_CONFIG_PATH')) include MODULE_CONFIG_PATH . 'config.php';
        else include $this->paths['root_code'] . $this->module . 'config.php';
        $this->_init_debug();
        if (!isset($this->db_connections) || !is_array($this->db_connections) || !count($this->db_connections)) $this->db_conn_enabled = false;
    }
    ...
    private function _init_redirects() {
        foreach ($this->redirects as $k => $v) if ($k . '/' == $this->query || $k == $this->query) $this->redirect(str_replace('//', $this->paths['root'], $v));
    }
    ...
    private function _init_title() {
        if (!$this->ajax) $this->title = isset($this->settings[$this->title_setting]) ? $this->settings[$this->title_setting]['value'] : $this->title;
    }
    ...
    private function _init_paths() {
        ...
        if (is_dir($this->subquery[0])) die('Inexistent module requested!');
        
        // component
        if (isset($this->subquery[2])) $this->component = $this->subquery[2];
        
        // subcomponent
        if (isset($this->subquery[3])) $this->subcomponent = $this->subquery[3];
        
        // components
        for ($i = 1; $i < count($this->subquery); $i++) if ($this->subquery[$i]) $this->components[] = $this->subquery[$i];
        if (!isset($this->components[0])) $this->components[0] = 'home';
        ...
        foreach ($this->subquery as $k => $v) {
            if ($v) {
                ...
                if ($this->paths['sub_paths'][$k][strlen($this->paths['sub_paths'][$k]) - 1] != '/') $this->paths['sub_paths'][$k].= '/';
            }
        }
        ...
        if (isset($this->paths['sub_paths'][2])) $this->paths['root_component'] = $this->paths['sub_paths'][2];
        if (isset($this->paths['sub_paths'][3])) $this->paths['root_subcomponent'] = $this->paths['sub_paths'][3];
    ...
    function save_page_settings() {
        if ($this->seo_enabled && $this->db_conn_enabled && !$this->ajax) {
            ...
            if ($this->libraries_settings['wbl_seo']['trackings_enabled']) {
                ...
                if (($this->libraries_settings['wbl_seo']['trackings_mode'] == 'all' || $this->libraries_settings['wbl_seo']['trackings_mode'] == 'robot') && $this->browser['type'] == 'robot') $params['crawler'] = $this->browser['browser'];
    ...
    private function _init_metas() {
        if (!$this->ajax && $this->seo_enabled) {
            ...
            foreach ($metas as $v) $this->set_meta_tag($v['name'], $v['content']);
        }
    }
    ...
    private function _init_language() {
        if (!isset($this->session['language_id'])) {
            if ($this->admin) {
                if (isset($this->settings['admin_default_language']['id'])) $this->session['language_id'] = $this->settings['admin_default_language']['id'];
                else $this->logger->log('settings_error', 'Setting field "admin_default_language" not found!');
            } else {
                if (isset($this->settings['default_language_id']['id'])) $this->session['language_id'] = $this->settings['default_language_id']['id'];
                else $this->logger->log('settings_error', 'Setting field "default_language_id" not found!');
            }
        }
        ...
        if ($this->multi_language) {
            ...
            if (strtolower($this->browser['os']) == 'windows' && isset_or($language['locale_win'])) setlocale(LC_ALL, $language['locale_win']);
            elseif (isset_or($language['locale_linux'])) setlocale(LC_ALL, $language['locale_linux']);
        }
    }
    ...
    private function _init_pagination() {
        if ($this->pagination_enabled) {
            ...
            // calculate page skip
            if ($this->page_no > 1) $this->page_skip = ($this->page_no - 1) * $this->page_offset;
        }
    }
    ...
    function set_meta_tag($name, $content) {
        if (isset($this->meta_tags[$name])) $this->meta_tags[$name]['content'] = $content;
        else $this->meta_tags[$name] = array(
            'name' => $name,
            'content' => $content
        );
    }
    ...
    function save_js_files() {
        ...
        foreach ($js_files as $k => $v) if (isset_or($v['src'])) {
            if ($v['local']) {
    ...
    function clear_cache($url = '') {
        if (!$url) $url = $this->paths['current_full'];
        if (class_exists('TemplatesManager')) TemplatesManager::clear_cache(base64_encode($url));
    }
    ...
    function change_template_dir($dir) {
        ...
        if (!is_dir(TemplatesManager::get_template_dir())) $this->logger->log('Templates_Error', 'Can not find templates directory "' . $dir . '"!');
    }
    ...
    function fetch_template($name, $file, $cache_folder, $return = false) {
        ...
        if (is_file($file)) {
            try {
                if ($return) return $this->template->fetch($file, $this->cache_hash);
                else $this->assign($name, $this->template->fetch($file, $this->cache_hash));
            }
            catch(Exception $ex) {
                trigger_error('Template Exception: ' . $ex->getMessage());
            }
        } else $this->logger->log('Templates_Error', 'Can not fetch template "' . $name . '" from file "' . $file . '"!');
    }
    ...
    function render_skin() {
        ...
        if (is_dir($this->paths['root_dir'] . $this->skins_folder . $this->skin . '/')) $skin_folder = $this->paths['root_dir'] . $this->skins_folder . $this->skin . '/' . $this->module;
    }
    ...
    function render_template() {
        ...
        if ($this->live && $this->render_type == 'all') $this->save_js_files();
        if (!$this->no_cache && (TemplatesManager::is_cached($this->paths['root_dir'] . $this->layout . '.tpl', $this->cache_hash) || TemplatesManager::is_cached(__DIR__ . '/objects/system/' . $this->layout . '.tpl', $this->cache_hash))) {
            ...
            if (is_file($template_folder . $this->layout . '.tpl')) $this->template->display($template_folder . $this->layout . '.tpl', $this->cache_hash);
            else $this->template->display(__DIR__ . '/objects/system/' . $this->layout . '.tpl', $this->cache_hash);
            die;
        }
        ...
        if (!TemplatesManager::is_cached($template_folder . $this->layout . '.tpl', $this->cache_hash)) {
            ...
            if (is_dir($this->paths['root_code'] . $this->module . 'views/' . $this->skin . '/')) $template_folder = $this->paths['root_code'] . $this->module . 'views/' . $this->skin . '/';
            elseif (is_dir($this->paths['root_code'] . $this->module . 'views/' . $this->default_skin . '/')) $template_folder = $this->paths['root_code'] . $this->module . 'views/' . $this->default_skin . '/';
            
            $cache_folder = $this->paths['root_cache'] . $this->module . 'views' . DS . $this->skin . DS;
            
            if (is_file($template_folder . 'noscript.tpl')) $this->fetch_template('__noscript', $template_folder . 'noscript.tpl', $cache_folder);
            if ($this->ajax && $this->obj_index->view != 'index') $this->render_type = 'page';
            if (is_a($this->obj_index, "Page")) $this->obj_index->_render_template($this->render_type);
            elseif (!$this->live && $this->build_enabled && isset($this->actions[0]) && $this->actions[0] == 'build-module') {
                ...
                if (!$build->add_module($this->paths['root_code'] . $this->module)) {
                    foreach ($build->errors as $e) $this->logger->log('builder_error', $e);
                }
                $this->redirect($this->paths['current']);
            } else trigger_error('No class named PageIndex extending Page provided in "index.php" file provided for module ' . $this->module);
        ...
        if (!headers_sent()) header('Content-Type: ' . $this->content_type);
        $this->change_cache_dir($cache_folder);
        try {
            if (is_file($template_folder . $this->layout . '.tpl')) $this->template->display($template_folder . $this->layout . '.tpl', $this->cache_hash);
            else $this->template->display(__DIR__ . '/templates/system/' . $this->layout . '.tpl', $this->cache_hash);
        }
        catch(Exception $ex) {
            trigger_error('Template Exception: ' . $ex->getMessage());
        }
    ...
    private function _get_render_type() {
        ...
        // check render type
        if ($this->render_all) return 'all';
        else {
            
            // check if it was changed
            if ($this->render_type != 'all') return $this->render_type;
            if (!count($this->components)) return 'page';
            else return 'page_component_' . (count($this->components) - 1);
        }
    }
    ...
    private function _404() {
        if ($this->enable_404) $this->_init_system_error('404', 'The page that you have requested could not be found.');
    }
    ...
    function render_scripts() {
        ...
        } else $this->import('class', 'objects.Page');
        if ($this->import('file', $this->paths['root_code'] . $this->module . 'index.php') && class_exists('PageIndex')) {
            ...
            if (!class_exists('PageIndex')) $this->logger->log('OOP_Warning', 'No \'PageIndex\' class found for this module!');
            else $this->_404();
        }
        ...
        try {
            ...
            // init index
            if (isset($this->obj_index)) $this->obj_index->_init();
            
            // render index
            if (!$this->restricted && isset($this->obj_index)) $this->obj_index->_render();
        }
        catch(Exception $ex) {
        ...
        if ($this->ajax && SessionManager::is_new() && $this->check_login) $this->redirect($this->paths['root_module']);
        ...
        if (is_dir($this->paths['root_objects_inc'])) {
            ...
            foreach ($files as $k => $v) {
                if ($v != '.' && $v != '..' && $v != '') {
                    ...
                    foreach ($objs as $l => $o) if ($o != '.' && $o != '..' && $o != '' && is_file($this->paths['root_objects_inc'] . $v . '/' . $o . '/include.tpl')) $this->objects[$v][$o] = $this->paths['root_objects_inc'] . $v . '/' . $o . '/include.tpl';
                }
            }
        }
    }
    ...
    private function _init_errors() {
        if (isset($this->session['errors']) && is_array($this->session['errors'])) foreach ($this->session['errors'] as $e) if (isset($e['field']) && isset($e['text'])) $this->errors[$e['field']] = $e['text'];
    }
    ...
    function init_session() {
        ...
        if (isset($this->session['state'])) $this->state = $this->session['state'];
        $this->clear_messages();
        $this->clear_errors();
    ...
    private function _init_check_cookies() {
        if ($this->check_cookies) {
            if ($this->actions[0] == '__no_cookies') $this->system_error = 'Please enable the cookies in your browser and then <a href="' . base64_decode($_REQUEST['page']) . '">click here</a> to go to the page you wanted to access.';
            elseif ($this->actions[0] == '__check_cookies') {
                $this->cookies_enabled = $_SESSION['__check_cookies'] && $_SESSION['_hash'] = $_REQUEST['hash'];
                if ($this->cookies_enabled) $this->redirect(base64_decode($_REQUEST['page']));
                else $this->redirect($this->paths['root_module'] . '?a=__no_cookies&page=' . $_REQUEST['page']);
            ...
            } else $this->cookies_enabled = true;
        }
    }
...
<script type="text/javascript">
    ...
    function set_module_user_type($type) {
        ...
        // check user
        if ($this->module_user_type && isset($this->session['user_type']) && $this->session['user_type'] != '' && $this->session['user_type'] != $this->module_user_type) $this->logout();
    }
    ...
    function add_message($type, $text) {
        if (@!is_array(@$this->session['messages'])) $this->session['messages'] = array();
    ...
    function add_error($field, $text) {
        if (!isset($this->session['errors']) || !is_array($this->session['errors'])) $this->session['errors'] = array();
    ...
    function set_query($query) {
        ...
        $this->module = $module;
        $pages[0] = $this->module;
        if (isset_or($pages[1])) $this->content = $pages[1];
        $this->subquery = $pages;
    }
     ...
     */
    function save_state() {
        if (isset($this->history[0])) $this->session['state_link'] = $this->history[0];
        else $this->session['state_link'] = $this->paths['current_full'];
    ...
    function clear_messages() {
        if (isset($this->session['messages'])) foreach ($this->session['messages'] as $k => $v) if ($v['showed'] >= 1) array_splice($this->session['messages'], $k, 1);
        else if (isset($this->session['messages'][$k]['showed'])) $this->session['messages'][$k]['showed']++;
    }
    ...
    function clear_errors() {
        if (isset($this->session['errors'])) {
            foreach ($this->session['errors'] as $k => $v) if ($v['showed'] >= 1) array_splice($this->session['errors'], $k, 1);
            else if (isset($this->session['errors'][$k]['showed'])) $this->session['errors'][$k]['showed']++;
            $this->_init_errors();
        }
    }
    ...
    function add_tables() {
        if (is_array($this->tables)) {
            $tables = new stdClass();
            foreach ($this->tables as $k => $v) $tables->$k = $v;
            $this->tables = $tables;
        }
    }
    ...
    private function _init_authenticate() {
        if (!isset($this->authenticate)) $this->authenticate = new AuthenticationManager();
    ...
    function add_validator($form_id, $field, $rule, $message = '', $client = false, $server = true) {
        if ($server) {
            ...
            if (!isset($this->session['validation'])) $this->session['validation'] = array();
            if (!isset($this->session['validation'][$hash])) $this->session['validation'][$hash] = array(
                'form_id' => $form_id,
                'fields' => array()
            );
    ...
    function get_page() {
        $arr = array();
        if ($this->trace) $arr['sys_version'] = $this->sys_version;
        $arr['hostname'] = self::$hostname;
        if ($this->trace) $arr['crypt_key'] = $this->crypt_key;
        ...
        if ($this->no_pages) for ($i = 1; $i <= $this->no_pages; $i++) $arr['pages'][$i] = $i;
     ...
     */
    function disconnect() {
        if (is_object($this->db_conn)) $this->db_conn->disconnect();
    }
    ...
    function add_config($name, $hostnames) {
        if (!isset($this->environments[$name])) $this->environments[$name] = array();
        if (is_array($hostnames)) foreach ($hostnames as $host) $this->environments[$name][] = $host;
        else $this->environments[$name][] = $hostnames;
    }
    ...
    function get_configs() {
        $found = array();
        self::get_hostname();
        foreach ($this->environments as $name => $hosts) foreach ($hosts as $host) if (is_callable($host) && $host($this)) $found[$name] = $name;
        elseif ($host == self::$hostname) $found[$name] = $name;
        return $found;
    }
}
?>